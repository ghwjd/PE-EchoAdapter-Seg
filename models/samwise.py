...
from models.sam_da_adapter import SAM_DA_Adapter # *
# from assets.sam_da_adapter_tmp import SAM_DA_Adapter # * 
...
def build_samwise(args):
    ...
    # --- [핵심 해결책] 모델 생성 후, 직접 어댑터 모듈 추가 ---
    if args.use_sam_da_adapter:
        print("\n[INFO] Post-Modifying model to insert SAM-DA Adapters...")
        # sam 모델 내부의 마스크 디코더 -> 트랜스포머 -> 레이어 목록에 접근합니다.
        for layer in sam.sam_mask_decoder.transformer.layers:
            # 각 TwoWayAttentionBlock에 어댑터 사용 플래그와 실제 어댑터 모듈을 추가합니다.
            layer.use_sam_da_adapter = True
            layer.adapter = SAM_DA_Adapter(
                embed_dim=layer.self_attn.embedding_dim,
                num_heads=layer.self_attn.num_heads,
                num_adapter_tokens=args.num_adapter_tokens,
            ).to(args.device) # 생성된 어댑터를 올바른 GPU 장치로 보냅니다.
    # ---------------------------------------------------------
    ...
    # 핵심 수정 2
    # 기본적으로 모든 파라미터를 학습 불가능(requires_grad=False)으로 설정합니다.
    # 파라미터 동결(Freeze) 로직
    # for param in model.parameters():
    #     param.requires_grad = False

    # if args.use_sam_da_adapter:
    #     print("\n[INFO] SAM-DA Adapter is enabled. Training only adapter parameters.")
    #     for name, param in model.named_parameters():
    #         if 'adapter' in name: 
    #             param.requires_grad = True
    # else:
    #     print("\n[INFO] SAM-DA Adapter is disabled. Training original SAMWISE adapters (CMT, CME).")
    #     for name, param in model.named_parameters():
    #         if 'cmt_adapters' in name or 'conditional_memory_encoder' in name or 'project_text' in name:
    #             param.requires_grad = True
    ...
    print("\n[INFO] Unfreezing base trainable parameters (CMT, CME, project_text)...")
    for name, param in model.named_parameters():
        if 'cmt_adapters' in name or 'conditional_memory_encoder' in name or 'project_text' in name:
            param.requires_grad = True

    if args.use_sam_da_adapter:
        print("[INFO] SAM-DA Adapter is enabled. Unfreezing adapter parameters ADDITIONALLY.")
        for name, param in model.named_parameters():
            if 'sam.sam_mask_decoder' in name and 'adapter' in name:
                param.requires_grad = True

    # 학습 가능한 파라미터 이름과 개수 출력 (디버깅용)
