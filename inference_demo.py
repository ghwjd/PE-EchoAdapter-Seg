...
def extract_frames_from_mp4(video_path):
    ...
    if os.path.isdir(extract_folder):
        ...
        extract_cmd = "ffmpeg -i {in_path} -loglevel error -vf fps=30 {folder}/frame_%05d.png"
...
def inference(args, model, save_path_prefix, in_path, text_prompts):
    ...
    for i in range(len(text_prompts)):
        ...
        if not args.image_level:
            ...
            clip = ImageSequenceClip(out_files_w_mask, fps=30)
...
def check_args(args):
    ...
    if not args.image_level: # it's video inference
        ...
        pretrained_model = '/ajeong/AISignal/SAMWISE/pretrain/final_model_camus_a6000_exp_warmup.pth'
