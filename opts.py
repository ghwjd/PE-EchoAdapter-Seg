...
def get_args_parser():
    ...
    parser.add_argument('--clip_max_norm', default=1, type=float,
                        help="Max norm for gradient clipping") # 1이 원래 값
    parser.add_argument("--loss_spike_thresh", type=float, default=0.5)
    parser.add_argument("--cme_spike_thresh",  type=float, default=0.5)
    # SAM-DA Adapter arguments
    parser.add_argument('--use_sam_da_adapter', action='store_true',
                        help="If True, enables the SAM-DA adapter in the mask decoder's transformer.")
    parser.add_argument('--num_adapter_tokens', default=64, type=int,
                        help="Number of learnable tokens for the SAM-DA adapter.")
    ...
    parser.add_argument('--dataset_file', default='ytvos', type=str,
                        help="Dataset to use: ['ytvos', 'davis', 'refcoco', 'refcoco+', 'refcocog', 'echonet', 'camus', 'all']")
    ...
    parser.add_argument('--echonet_path', type=str, default='data/echonet',
                        help="Path to EchoNet dataset")
    parser.add_argument('--camus_path', type=str, default='data/camus',
    ...
    parser.add_argument('--split', default='valid', type=str, choices=['valid', 'valid_u', 'test', 'val'],
    ...
    parser.add_argument('--frames_only', action='store_true',
                    help="GT/JSON 없이 프레임만 있는 폴더에서 마스크만 저장")
    parser.add_argument('--caption', default="left ventricle endocardium",
                        help="텍스트 프롬프트(기본: LV endocardium)")
